...
const getTagsStatus = async (tags: number[]) => {
  // 批量获取所有标签以提高性能
  const tagPromises = tags.map(async index => {
    try {
      const tag = await tagManager.getTag(index);
      if (tag) {
        return `<span data-v-43a42535="" data-v-c06fccc2="" class="lfe-caption" data-v-303bbf52="" style="color: rgb(255, 255, 255); background-color: ${tag.color}">${tag.name}</span>&nbsp;`;
      } else {
        return `<span data-v-43a42535="" data-v-c06fccc2="" class="lfe-caption" data-v-303bbf52="" style="color: rgb(255, 255, 255); background-color: #666666">未知标签(${index})</span>&nbsp;`;
      }
    } catch (error) {
  ...
  const tagHtmls = await Promise.all(tagPromises);
